...
for selectedDataset in datasets:
    ...
    for image_index,image_name in enumerate(textCSV):
        ...
        if not os.path.exists(samus_path):
            print(samus_path)
            continue
        ...
        if image_source.shape[0]!=samus_mask.shape[0] or image_source.shape[1]!=samus_mask.shape[1]:
            image_source = cv2.resize(image_source.astype(np.uint8), (samus_mask.shape[1], samus_mask.shape[0]), interpolation=cv2.INTER_NEAREST)

        if mask_source.shape[0]!=samus_mask.shape[0] or mask_source.shape[1]!=samus_mask.shape[1]:
            mask_source = cv2.resize(mask_source.astype(np.uint8), (samus_mask.shape[1], samus_mask.shape[0]), interpolation=cv2.INTER_NEAREST)

        samus_mask[samus_mask>=threshold]=1
        samus_mask[samus_mask<threshold]=0


        iou = sklearn_iou(samus_mask,mask_source)*100
        dic = sklearn_dice(samus_mask,mask_source)*100
        if show_plots:
