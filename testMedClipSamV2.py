...
for image_index,image_name in enumerate(textCSV):
    ...
    sam_path = f'multimodal-data/MedClipSamResults/MedCLIP-SAMv2/{selectedDataset}/{image_name}'.replace('png','npz').replace('jpg','npz')
...
with open(f'{save_result_path}/result.txt', 'w') as f:
    f.write(f"Average IoU: {ious.mean():.2f}±{ious.std():.2f}\n")
    f.write(f"Average Dic: {dices.mean():.2f}±{dices.std():.2f}\n")
# %%
