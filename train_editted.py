...
import torch
# torch.cuda.set_device(1)                # switch current device
...
def train(config_path: str, save_dir: Optional[str] = None) -> None:
    ...
    visualizer = GroundingDINOVisualizer(save_dir=save_dir, visualize_frequency=5)
    ...
    for epoch in range(training_config.num_epochs):
        ...
        if (epoch!=0) and epoch % training_config.visualization_frequency == 0:
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            losses_train = trainer.train_step(batch)
            # Record losses
            for k, v in losses_train.items():
                epoch_losses_train[k].append(v)
                writer.add_scalar(f"loss/train/{k}", v, global_step)
            current_lr = trainer.optimizer.param_groups[0]["lr"]
            writer.add_scalar("lr", current_lr, global_step)
            global_step += 1
         
            if batch_idx % 5 == 0:
                loss_str = ", ".join(f"{k}: {v:.4f}" for k, v in losses_train.items())
                print(f"Epoch {epoch+1}/{training_config.num_epochs}, "
                      f"Batch {batch_idx}/{len(train_loader)}, {loss_str}")
                print(f"Learning rate: {trainer.optimizer.param_groups[0]['lr']:.6f}")

        avg_losses_train = {k: sum(v)/len(v) for k, v in epoch_losses_train.items()}
        print(f"Epoch {epoch+1} complete. Average losses:",
              ", ".join(f"{k}: {v:.4f}" for k, v in avg_losses_train.items()),f"Er:{early_stop}")
        avg_losses_val = trainer.validate(val_loader)
        print(f"Epoch {epoch+1} complete. Average validation losses:",
              ", ".join(f"{k}: {v:.4f}" for k, v in avg_losses_val.items()),f"Er:{early_stop}")
        
        for k, v in avg_losses_train.items():
            writer.add_scalar(f"epoch_loss/train/{k}", v, epoch + 1)

        for k, v in avg_losses_val.items():
            writer.add_scalar(f"epoch_loss/val/{k}", v, epoch + 1)
        # Save Best Model
        if avg_losses_val['total_loss'] < best_val_loss:
            early_stop=0
            best_val_loss = avg_losses_val['total_loss']
            print(f"New best model found at epoch {epoch+1}, saving...")
            trainer.save_checkpoint(
                os.path.join(save_dir, f'best_model.pth'),
                epoch,
                avg_losses_val['total_loss'],
                use_lora=training_config.use_lora
            )
        
        early_stop+=1
        
        if early_stop==training_config.patient:
            print(f'Stop after {early_stop} epoch without improvement')
            break
