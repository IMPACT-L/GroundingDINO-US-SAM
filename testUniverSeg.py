...
for selectedDataset in datasets:
    ...
    for image_index,image_name in enumerate(textCSV):
        ...
        # universeg_path = f'multimodal-data/MedClipSamResults/MedCLIP-SAMv2/{selectedDataset}/{image_name}'.replace('png','npz').replace('jpg','npz').replace('.bmp','.npz')
        data = np.load(universeg_path)
        universeg_mask = data['arr']  # Replace 'array_name' with actual key from data.files
        
        universeg_mask[universeg_mask>=threshold]=1
        universeg_mask[universeg_mask<threshold]=0

        if universeg_mask.shape[0]!=image_source.shape[0] or universeg_mask.shape[1]!=image_source.shape[1]:
            mask_source = cv2.resize(mask_source.astype(np.uint8), (universeg_mask.shape[1], universeg_mask.shape[0]), interpolation=cv2.INTER_NEAREST)

        if image_source.shape[0]!=universeg_mask.shape[0] or image_source.shape[1]!=universeg_mask.shape[1]:
            image_source = cv2.resize(image_source.astype(np.uint8), (universeg_mask.shape[1], universeg_mask.shape[0]), interpolation=cv2.INTER_NEAREST)


        iou = sklearn_iou(universeg_mask,mask_source)*100
        dic = sklearn_dice(universeg_mask,mask_source)*100
        if show_plots:
            ...
            tmp_image = image_source.copy()
            tmp_image[:,:,2][universeg_mask==1]=255
