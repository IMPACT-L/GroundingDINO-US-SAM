...
for mask_path in mask_paths:
    ...
    for _, contour in enumerate(contours):
        ...
        row =[
            type,
            x, y, w, h,
            f"{dataset}_{file_name.replace('General-1-channel-Labels','BreastTumourImages').replace('png','jpg')}",
            mask.shape[1],
            mask.shape[0],
            mask_path,
            dataset
...
def copyImages(data_in, output_type):
    for data in data_in:
        image_name = data[5].replace(dataset+'_','').replace('BreastTumourImages','General-1-channel-Labels').replace('png','jpg')
#             f"{dataset}_{file_name.replace('General-1-channel-Labels','BreastTumourImages').replace('png','jpg')}",
        img_path = f'{srcDir}/BreastTumourImages/{image_name}'
